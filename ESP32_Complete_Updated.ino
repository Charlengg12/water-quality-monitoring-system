...
// Fixed: Using dashboard.php with station_id parameter (matching dashboard.php expectations)
const char* serverBaseUrl = "https://water.monitoring.ehub.ph/pages/dashboard.php";
const int STATION_ID = 41; // Change this to match your station ID
...
void sendDataToServer() {
    ...
    for (int attempt = 0; attempt < MAX_RETRIES; attempt++) {
        ...
        HTTPClient http;
        WiFiClientSecure client; // Use HTTPS client for secure connection
        client.setInsecure(); // Bypass certificate validation (use with caution)
        
        // Build URL with station_id parameter (matching dashboard.php expectations)
        String fullUrl = String(serverBaseUrl) + "?station_id=" + String(STATION_ID);
        
        Serial.printf("\nAttempt %d: Uploading to %s\n", attempt + 1, fullUrl.c_str());
        
        http.begin(client, fullUrl); // Use secure client for HTTPS
        http.addHeader("Content-Type", "application/json");
        
        // Create JSON payload matching dashboard.php format (lowercase with underscores)
        StaticJsonDocument<512> doc;
        doc["sensorId"] = sensorId;
        doc["tds_val"] = tdsValue;
        doc["ph_val"] = phValue;
        doc["turbidity_val"] = turbidityValue;
        doc["lead_val"] = leadValue;
        doc["color_val"] = tcuValue;
...
void handleReadings() {
    ...
    // Match dashboard.php JavaScript expectations (PascalCase with first letter capitalized only)
...
void setup() {
    ...
    if (!tcs.begin()) {
        ...
        Serial.println("1. Sensor is in a stable position");
        Serial.println("2. Lighting conditions are consistent");
        Serial.println("3. No objects blocking the sensor");
        Serial.println("========================================");
        ...
        // 20-second calibration countdown
        for (int i = 20; i > 0; i--) {
            lcd.setCursor(0, 2);
            lcd.printf("Wait: %2d seconds", i);
            Serial.printf("Calibration: %2d seconds remaining...\r", i);
            delay(1000);
            server.handleClient(); // Keep web server responsive
        }

        Serial.println("\nCalibration complete!                    ");
        Serial.println("Color sensor ready for measurements.\n");
    ...
    server.on("/start_test", HTTP_OPTIONS, handleStartTest); // Handle CORS preflight
    server.onNotFound([](){
        server.sendHeader("Access-Control-Allow-Origin", "*");
        ...
        case DISPLAY_READINGS:
            if (currentMillis - displayGroupTimer >= DISPLAY_GROUP_TIME_MS) {
                ...
                if (displayGroupIndex <= 4) {
                    ...
                    // All readings displayed, now show on Serial Monitor
                    Serial.println("\n========================================");
                    ...
                    // Wait 5 seconds to review data on serial monitor
                    Serial.println("\nWaiting 5 seconds before upload...");
                    for (int i = 5; i > 0; i--) {
                        Serial.printf("Uploading in %d seconds...\n", i);
                        delay(1000);
                        server.handleClient(); // Keep web server responsive during delay
                    }
                    ...
                    if (isWiFiConnected) {
                        ...
                        Serial.println("\nSkipping upload due to no WiFi.");
                    }
